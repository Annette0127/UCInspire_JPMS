...
import edu.usc.softarch.arcade.topics.*;
...
public class ArchSmellDetector {
	...
	private static void detectSpfNew(
		...
		for (Entry<Integer, Integer> entry : topicNumCountMap.entrySet()) {
			logger.debug(entry.getKey() + " : " + entry.getValue() );
		}
		logger.debug("topic num : clusters with topic");
		for (Entry<Integer, Set<ConcernCluster>> entry : scatteredTopicToClustersMap.entrySet()) {
			logger.debug(entry.getKey() + " : " + entry.getValue() );
		}
